...
Enemy = function(id,x,y,width,height,img,hp,atkSpd,letter){
	...
	newFunction = function(self.letter){
...
Enemy.randomlyGenerate = function(){
	...
	if(Math.random() < 0.30) //id,x,y,width,height,img,hp,atkSpd
		Enemy(id,x,y,width,height,Img.FOne,2,0.1,'21');
	else if (0.3 < Math.random() < 0.55)
		Enemy(id,x,y,width,height,Img.FTwo,1,0.2,'22');
	else if (0.55 < Math.random() < 0.75)
		Enemy(id,x,y,width,height,Img.FThree,1,0,23);
	else if (0.75< Math.random() < 0.90)
		Enemy(id,x,y,width,height,Img.FFour,1,0,24);
	else if (0.90 < Math.random() < 0.99)
		Enemy(id,x,y,width,height,Img.FFive,1,0,25);
...
nextLetterSpot is a counter
letterSpots is an array of objects with x,y, and image attributes to display pw
pw_Input is the string generated by the player

updatePW = function(bulletType, letter, nextLetterSpot){


	if (bulletType == addBullet){
		letterSpots[nextLetterSpot].img = '_img/'+letter+'.png';
		pw_Input.push(letter);
		...
		if (pw_Input.length>0){
			nextLetterSpot--;
			letterSpots[nexLetterSpot-1].img = '_img/blankLetter.png';
